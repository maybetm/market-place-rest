 ...
 */
public class TestCaseForDefaultMarket {
  ...
  public TestCaseForDefaultMarket(Account marketRQ, MockMvc mockMvc) {
	...
	*/
	public void process() throws Exception {
		...
		Assert.assertEquals("Удаление товара. Неверный HTTP статус",
				this.deleteProduct(2L, marketTokenRS.getToken()), HttpStatus.OK.value());
		Assert.assertEquals("Попытка получить информацию по удалённому товару. Неверный HTTP статус",
		...
		Assert.assertEquals("Обновление количества выбранного товара. Неверный HTPP статус",
		...
		Assert.assertEquals("Попытка удалить токен. Неверный HTPP статус.",
